import React , {useState , useEffect } from 'react'
import DossiersReferentielNavbar from '../DossiersReferentielNavbar';
import '../../../../App.css'
import Grid from '@mui/material/Grid';
import TextField from '@mui/material/TextField';
import Button from "@mui/material/Button"
import axios from 'axios';
import Alert from '@mui/material/Alert';
import { useParams } from "react-router-dom";

export default function EditCouleur() {

  let { codeCouleur , nomCouleur } = useParams();

  var couleurID = '';

  const buttonStyle = {  marginTop: "10px" }
  const [couleurName,setNomCouleur] = useState(nomCouleur)
  const [couleurCode,setCodeCouleur] = useState(codeCouleur)

  const [successAlert, setSuccessAlert] = useState(false);
  const [successAlertContent, setSuccessAlertContent] = useState('');

  const [errorAlert, setErrorAlert] = useState(false);
  const [errorAlertContent, setErrorAlertContent] = useState('');

  axios.get(`http://localhost:8080/couleurs/getID/${codeCouleur}`)
  .then(function (response) {
    couleurID = response.data
  })

  const handleClick=(e)=>{
      e.preventDefault()
      axios.put('http://localhost:8080/couleurs/update/'+couleurID+'/'+couleurCode+'/'+couleurName , {
      })
      .then(res => {
          console.log(res);
          console.log(res.data);
          if(res.data == "Couleur modifiée")
          {
              setSuccessAlertContent(res.data);
              setSuccessAlert(true);
              setErrorAlert(false);
          }
          else if(res.data == "Couleur non modifiée")
          {
              setErrorAlertContent(res.data);
              setErrorAlert(true);
              setSuccessAlert(false);
          }
      })
  }

  return (
    <div>
      <DossiersReferentielNavbar/>
      <h1>Modifier une couleur</h1>
      {successAlert ? <Alert severity="success" className='alert'>{successAlertContent}</Alert> : <></> }
      {errorAlert ? <Alert severity="error" className='alert'>{errorAlertContent}</Alert> : <></> }

      <React.Fragment>
          <Grid container className='content'>
              <Grid item xs={6} sm={6}>
                  <TextField
                    required
                    id="couleurCode"
                    name="couleurCode"
                    label="Code de la couleur"
                    fullWidth
                    autoComplete="given-name"
                    variant="standard"
                    value={couleurCode}
                    onChange={(e)=>setCodeCouleur(e.target.value)}/>

                  <TextField
                    required
                    id="couleurName"
                    name="couleurName"
                    label="Nom de la couleur"
                    fullWidth
                    autoComplete="given-name"
                    variant="standard"
                    value={couleurName}
                    onChange={(e)=>setNomCouleur(e.target.value)}/>
              </Grid>
              <Grid item xs={12} style={buttonStyle}>
                  <Button variant="contained" onClick={handleClick}>Enregistrer</Button>
              </Grid>
        </Grid>
      </React.Fragment>
    </div>
  )
}
