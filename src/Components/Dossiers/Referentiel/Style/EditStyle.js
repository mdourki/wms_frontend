...
export default function EditStyle() {
    ...
    const [styleName,setStyleName] = useState(style)
    ...
    axios.get(`http://localhost:8080/styles/getID/${style}`)
    ...
    const handleClick=(e)=>{
        e.preventDefault()
        axios.put('http://localhost:8080/styles/update/'+styleID+'/'+styleName , {
        })
        .then(res => {
            console.log(res);
            console.log(res.data);
            if(res.data == "Style modifié")
            {
                setSuccessAlertContent(res.data);
                setSuccessAlert(true);
                setErrorAlert(false);
            }
            else if(res.data == "Style non modifié")
            {
