...
export default class ListCategoriesFournisseur extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8080/listCatFour/totalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getCategories = (page , size) => {
        axios.get('http://localhost:8080/listCatFour/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    deleteCat(categorieName) {
        axios.get(`http://localhost:8080/listCatFour/getID/`+categorieName)
        .then(response => {
            this.setState({
    ...
    deleteCatFromDB() {
        axios.delete('http://localhost:8080/listCatFour/delete/'+this.state.fourCategorieID)
        .then(response => { 
            if(response.data == "Impossible de supprimer cette catégorie, car elle est associée à un fournisseur")
            {
                this.setState({
  ...
  render() {
    return (
        <div>
            ...
            <h1>Liste Catégories Fournisseur</h1>
            {this.state.errorAlert ? <Alert severity="error" className='alert'>{this.state.errorAlertContent}</Alert> : <></> }
            <div className='content'>
                
                <Link to='/dossiers/categories/categoriesFour/New' className='nav-links'>
                    ...
                    <Table sx={{ minWidth: 900 }} aria-label="simple table">
                        ...
                        {this.state.categories.map(categorie => (
                            <TableRow key={categorie}>
                                ...
                                <TableCell>
                                    <Link to={'/dossiers/categories/categoriesFour/edit/'+categorie} >
