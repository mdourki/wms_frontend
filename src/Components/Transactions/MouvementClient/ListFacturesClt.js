...
export default class ListFacturesClt extends Component {
    ...
    getTotalPages = (page,size) => {
        axios.get('http://localhost:8080/ligneFactClt/totalPages/'+page+'/'+size)
        .then(response => {
            this.setState({
    ...
    getFactures = (page , size) => {
        axios.get('http://localhost:8080/ligneFactClt/'+page+'/'+size)
        .then(response => {
            this.setState({
                ...
                <Table sx={{ minWidth: 900 }} aria-label="simple table">
                    ...
                    {this.state.factures.map(facture => (
                        <TableRow key={facture[0]}>
                            <TableCell>
                                {"Facture Livraison "+facture[1]}
                            </TableCell>
                            <TableCell>
                                <Link to={'/transactions/mvClt/facturation/print/'+facture[0]} >
